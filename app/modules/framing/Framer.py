        ...
        for gesture in range(len(self.gestures)):
            ...
            for dimension in range(3):
                ...
                if captures[-1][-1][-1].state == States.stable:
                    ...
                    captures[-1][-1][-1].end_time = last_time_state.timestamp
                    captures[-1][-1][-1].end_value = last_time_state.rotation[dimension]
            for dimension in range(3):
                captures[-1].append([])
                for time_state in range(len(self.gestures[gesture].samples[0].time_states) - 1):
                    old_time_state = self.getTimeState(gesture, time_state)
                    new_time_state = self.getTimeState(gesture, time_state + 1)
                    if len(captures[-1][-1]) == 0:
                        if new_time_state.acceleration[dimension] - old_time_state.acceleration[dimension] >= TO_RISING:
                            captures[-1][-1].append(
                                Capture(old_time_state.timestamp, old_time_state.acceleration[dimension], States.rising))
                        else:
                            captures[-1][-1].append(
                                Capture(old_time_state.timestamp, old_time_state.acceleration[dimension], States.falling))
                    else:
                        if captures[-1][-1][-1].state == States.rising:
                            if new_time_state.timestamp - old_time_state.timestamp < FROM_RISING_TO_FALLING:
                                captures[-1][-1][-1].end_time = old_time_state.timestamp
                                captures[-1][-1][-1].end_value = old_time_state.acceleration[dimension]
                                captures[-1][-1].append(
                                    Capture(new_time_state.timestamp, new_time_state.acceleration[dimension], States.falling))
                            elif new_time_state.timestamp - old_time_state.timestamp < FROM_RISING_TO_STABLE:
                                captures[-1][-1][-1].end_time = old_time_state.timestamp
                                captures[-1][-1][-1].end_value = old_time_state.acceleration[dimension]
                                captures[-1][-1].append(
                                    Capture(new_time_state.timestamp, new_time_state.acceleration[dimension], States.stable))
                        elif captures[-1][-1][-1].state == States.stable:
                            if new_time_state.timestamp - old_time_state.timestamp >= FROM_STABLE_TO_RISING:
                                captures[-1][-1][-1].end_time = old_time_state.timestamp
                                captures[-1][-1][-1].end_value = old_time_state.acceleration[dimension]
                            ...
                            if new_time_state.timestamp - old_time_state.timestamp >= FROM_FALLING_TO_RISING:
                                ...
                                captures[-1][-1][-1].end_value = old_time_state.acceleration[dimension]
                                captures[-1][-1].append(
                                    Capture(new_time_state.timestamp, new_time_state.acceleration[dimension], States.rising))
                            elif new_time_state.timestamp - old_time_state.timestamp >= FROM_FALLING_TO_STABLE:
                                captures[-1][-1][-1].end_time = old_time_state.timestamp
                                captures[-1][-1][-1].end_value = old_time_state.acceleration[dimension]
                                captures[-1][-1].append(
                ...
                if captures[-1][-1][-1].state == States.stable:
                    ...
                    last_time_state = self.getTimeState(gesture, len(self.gestures[gesture].samples[0].time_states) - 1)
                    captures[-1][-1][-1].end_time = last_time_state.timestamp
                    captures[-1][-1][-1].end_value = last_time_state.acceleration[dimension]
        return captures
    ...
    def getTimeState(self, gesture, time_state):
        ...
        for tmp_time_state in range(len(time_states) - 1):
            ...
            if current_time_state.timestamp == time:
                ...
                diff_rotation = numpy.subtract(next_time_state.rotation, current_time_state.rotation)
                diff_acceleration = numpy.subtract(next_time_state.acceleration, current_time_state.acceleration)
                return TimeState(None, None,
                                 numpy.add(current_time_state.rotation, numpy.multiply(diff_rotation, div)),
                                 numpy.add(current_time_state.acceleration, numpy.multiply(diff_acceleration, div)),
                                 time - self.gesture_start[0])
